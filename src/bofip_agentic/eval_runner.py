...
def auto_verdict(result: PerQueryResult) -> str:
    ...
    answer_text = "\n".join(
        ...
        for part in [
            result.conclusion,
            result.limits,
            "\n".join(result.justification_bullets),
        ]
        if part
    )
    if result.answer_status == "insufficient_evidence" or _looks_like_abstention(answer_text):
        return "candidate_fail_insufficient_evidence"
    trace_ok = result.scores.trace_score >= 0.75
    has_answer_gold = bool(result.scores.answer_point_hits or result.scores.missing_answer_points)
    answer_gold_acceptable = not has_answer_gold or result.scores.answer_point_recall >= 0.4
    has_required_gold = bool(result.scores.required_doc_hits or result.scores.missing_required_docs)
    has_key_source = bool(result.scores.required_doc_hits) or not has_required_gold
    has_bofip_source = any(str(source.boi_reference).upper().startswith("BOI-") for source in result.sources)
    if result.answer_status not in {"supported", "partial"}:
        return "needs_human_review_unknown_status"
    if has_key_source and answer_gold_acceptable:
        return "candidate_pass"
    if has_key_source and result.coverage >= 0.5 and trace_ok:
        return "candidate_pass"
    if has_bofip_source and answer_gold_acceptable and result.coverage >= 0.5 and trace_ok:
        return "candidate_pass"
    return "needs_human_review"


def _looks_like_abstention(answer_text: str) -> bool:
    normalized = _normalized_phrase(answer_text)
    if not normalized:
        return True
    abstention_markers = [
        "aucune reponse ne peut etre donnee",
