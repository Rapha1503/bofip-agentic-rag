...
def _clean_answer_status(answer: dict, plan: SearchPlan | None = None, source_review: dict | None = None) -> dict:
    ...
    has_concrete_answer = _has_principal_answer_signal(answer) and not _is_abstention_answer(answer)
...
def _is_abstention_answer(answer: dict) -> bool:
    text_parts = [
        str(answer.get("answer_status", "") or ""),
        str(answer.get("conclusion", "") or ""),
    ]
    text_parts.extend(str(item) for item in answer.get("justification_bullets", []) or [])
    text = _ascii_lower("\n".join(text_parts))
    markers = (
        ...
        "aucun extrait ne prouve",
        "aucune source ne prouve",
