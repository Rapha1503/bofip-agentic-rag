...
def _status_meta(status: str | None) -> tuple[str, str, str]:
    mapping = {
        ...
        "partial": ("partial", "Réponse avec réserve", "Le statut partiel est conservé seulement pour compatibilité avec d'anciens résultats."),
    ...
    st.markdown(
        ...
        <div class="app-shell">
          ...
          <div class="system-strip">
            <div class="system-item"><span>Corpus</span><strong>9 048 documents</strong><small>Source BOFiP observée jusqu'au 17/06/2026</small></div>
...
with config_col:
    with st.container(border=True):
        ...
        if requires_api_key:
            ...
            api_key = ""
            st.markdown(
                '<p class="field-note">CLI Codex local authentifié. Aucun champ clé API requis.</p>',
                unsafe_allow_html=True,
            )
        ...
        if provider.get("note"):
            st.markdown(f'<p class="field-note">{_escape(provider["note"])}</p>', unsafe_allow_html=True)
        st.markdown(
            f'<p class="field-note">Recherche locale: {_escape(retrieval_profile.label)}.</p>',
            unsafe_allow_html=True,
        )
        use_reranker = False
