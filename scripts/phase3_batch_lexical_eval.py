...
def main() -> int:
    parser = argparse.ArgumentParser(description="Phase 3A batch lexical evaluation.")
    ...
    report_path = (
        ...
        if args.output
        else REPORTS_DIR / f"phase3_batch_eval_{Path(args.chunks).stem}__{Path(args.queries).stem}.json"
    )
    write_json(report_path, summary)

    print(f"Batch lexical evaluation complete: {report_path}")
    for k in ks:
